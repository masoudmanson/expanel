<?php
/**
 * Created by PhpStorm.
 * User: pooria
 * Date: 12/12/16
 * Time: 7:46 PM
 */

return [
    'post_image' => public_path().'/images/post_imgs/',
    'post_cover' => public_path().'/images/post_covers/',
    'post_video' => public_path().'/videos/post_videos/',
    'post_audio' => public_path().'/audios/post_audios/',
    'post_pdf' => public_path().'/pdf/post_pdfs/',

    'client_image' => public_path().'/images/client_imgs/',
    'gallery_image' => public_path().'/images/gallery_imgs/',
    'gallery_cover' => public_path().'/images/gallery_covers/',
    'gallery_video' => public_path().'/videos/gallery_videos/',
];